...
class SurveyRedirectionTest(TestCase):
    ...
    def test_show_message_verb_on_sms(self):
        response = self.client.get(reverse('survey', kwargs={'survey_id': self.survey.id}),
                                   {'MessageSid': '123'})
...
class SurveyResultsTest(TestCase):
    ...
    def setUp(self):
        ...
        self.survey.save()
        self.question = Question(body='Question one', kind=Question.TEXT, survey=self.survey)
        self.question.save()
        QuestionResponse(response='A response',
                         message_sid='sup3runiqu3',
                         phone_number='+14155552671',
                         question=self.question).save()

    def test_redirect_to_results(self):
        redirect = self.client.get(reverse('app_root'))
        survey_results_url = reverse('survey_results', kwargs={'survey_id': self.survey.id})
